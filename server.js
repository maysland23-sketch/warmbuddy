...
if (OUTBOUND_PROXY) {
  console.log('[proxy] Outbound proxy configured');
}
...
if (IS_RENDER) {
  const publicOrigin = new URL(APP_PUBLIC_ORIGIN);
  if (!['http:', 'https:'].includes(publicOrigin.protocol) || publicOrigin.pathname !== '/' || publicOrigin.search || publicOrigin.hash) {
    throw new Error('APP_PUBLIC_ORIGIN must be an HTTP(S) origin');
  }
}
...
const NTFY_BASE_URL = (process.env.NTFY_BASE_URL || 'https://ntfy.sh').trim();
const NTFY_TOPIC = (process.env.NTFY_TOPIC || '').trim();
const NTFY_TOKEN = (process.env.NTFY_TOKEN || '').trim();
const NTFY_CLICK_BASE_URL = (process.env.NTFY_CLICK_BASE_URL || APP_PUBLIC_ORIGIN || 'https://warmbuddy.vercel.app').trim().replace(/\/+$/, '');
console.log('[ntfy] Channel:', NTFY_TOPIC ? `configured (${NTFY_BASE_URL})` : 'not configured');

// Log only configuration state; never log key material or fingerprints.
console.log('[vapid] keys configured:', Boolean(VAPID_PUBLIC_KEY && VAPID_PRIVATE_KEY));
...
if (VAPID_PUBLIC_KEY && VAPID_PRIVATE_KEY) {
  ...
  if (result.ok) {
    ...
    console.error('❌ VAPID env keys failed');
...
if (!vapidInitialized) {
  ...
  if (genResult.ok) {
    ...
    console.log('   Add VAPID_PUBLIC_KEY and VAPID_PRIVATE_KEY to Vercel env vars for stability.');
  } else {
    console.error('❌ VAPID auto-generation also failed');
...
if (!IS_RENDER) app.use(express.static('public'));
...
async function updateEventNtfyState(eventId, fields) {
  if (!supabase || !eventId) return;
  const { error } = await supabase.from('system_events').update(fields).eq('id', eventId);
  if (error) throw error;
}

async function markEventNtfyAttempt(eventId, attempt) {
  await updateEventNtfyState(eventId, {
    ntfy_attempts: attempt,
    ntfy_last_error: null
  });
}

async function markEventNtfySent(eventId, result) {
  await updateEventNtfyState(eventId, {
    ntfy_sent: true,
    ntfy_sent_at: new Date().toISOString(),
    ntfy_last_error: null,
    ntfy_message_id: result && result.id ? String(result.id) : null
  });
}

async function markEventNtfyFailed(eventId, error) {
  await updateEventNtfyState(eventId, {
    ntfy_sent: false,
    ntfy_last_error: error ? String(error.message || error).slice(0, 1000) : 'Unknown ntfy error'
  });
}

const ntfyDispatcher = createNotificationDispatcher({
  baseUrl: NTFY_BASE_URL,
  topic: NTFY_TOPIC,
  token: NTFY_TOKEN,
  maxAttempts: 3,
  retryDelayMs: 500,
  markAttempt: markEventNtfyAttempt,
  markSent: markEventNtfySent,
  markFailed: markEventNtfyFailed,
  onStatusError: error => console.error('[ntfy] Status persistence failed:', error.message)
});
...
app.post('/api/test-connection', async (req, res) => {
  ...
  console.log(`[test-conn] model=${testModel} format=${format} provider=${resolved.provider} endpointHost=${safeLogUrlHost(fetchUrl)}`);
  ...
  try {
    ...
    console.log('[test-conn] request prepared', {
    ...
    if (!response.ok) {
      ...
      console.error(`[test-conn] Resolved: provider=${resolved.provider} format=${format} endpointHost=${safeLogUrlHost(fetchUrl)}`);
      return res.json({
        success: false,
        provider: resolved.provider,
        format,
    ...
    console.error('[test-conn] request failed', {
      name: err?.name || 'Error',
...
async function ensureMCPSession(url, auth) {
  ...
  try {
    ...
    if (sessionId) {
      ...
      console.log('[mcp] Session initialized:', safeLogUrlHost(url));
      return sessionId;
    }
...
async function mcpRequest(url, sessionId, method, params, auth) {
  ...
  console.log('[mcp-debug] mcpRequest prepared', {
    host: safeLogUrlHost(url),
    method,
    sessionConfigured: Boolean(sessionId),
    authConfigured: Object.keys(authHeaders).length > 0
  });
  ...
  try {
    ...
    try {
      ...
      if (json.error) {
        ...
        console.warn('[mcp] JSON-RPC error:', json.error.code || 'UNKNOWN');
        return null;
      }
      ...
      console.log('[mcp-debug] mcpRequest → JSON parse failed, trying SSE fallback');
...
async function listMCPTools(toolDef) {
  ...
  if (cached && cached.expiresAt > Date.now()) {
    console.log('[mcp] Using cached tool list for', safeLogUrlHost(url), '(' + cached.tools.length + ' tools)');
    return cached.tools;
  }
  ...
  console.log('[mcp] Listed', tools.length, 'tool(s) from', safeLogUrlHost(url));
  return tools;
}
...
async function getEnabledToolDefsForWindow(projectId, windowId, clientEnabledIds, clientToolDefs) {
  try {
    ...
    console.warn('[mcp] getEnabledToolDefsForWindow error:', err?.code || err?.name || 'UNKNOWN');
    return [];
  }
}
...
app.post('/api/chat/stream', async (req, res) => {
  try {
    ...
    console.log(`[chat/stream] model=${model} format=${format} provider=${resolved.provider} endpointHost=${safeLogUrlHost(fetchUrl)}`);
    ...
    if (!response.ok) {
      ...
      let errMsg = errText;
      try { const ej = JSON.parse(errText); errMsg = ej.error?.message || ej.error?.code || errText; } catch {}
      console.error(`[chat/stream] upstream error: status=${response.status}`);
      res.write(`data: ${JSON.stringify({ error: `API error (${response.status}): ${errMsg}` })}\n\n`);
      res.end();
      return;
    }
    ...
    while (true) {
      ...
      for (const line of lines) {
        ...
        if (trimmed.startsWith('data: ')) {
          ...
          if (DEBUG_STREAM) console.log('[chat/stream] non-data line received');
          skippedLines++;
          continue;
        }
    ...
    if (buffer.trim()) {
      ...
      if (data !== '[DONE]' && data !== 'data: [DONE]') {
        ...
        if (parsed) {
          ...
          if (parsed.usage) streamUsage = mergeUsageSnapshots(streamUsage, parsed.usage);
          if (parsed.text) streamText += parsed.text;
          // Forward parsed data without logging its contents.
          res.write(`data: ${JSON.stringify(parsed)}\n\n`);
        }
      }
    }
    ...
    if (chunkCount < 2) {
      console.log(`[chat/stream] WARNING: only ${chunkCount} chunk(s) forwarded`);
    ...
    if (chunkCount === 0) {
      ...
      try {
        ...
        if (fallbackResp.ok) {
          ...
          let errMsg = errText;
          try { const ej = JSON.parse(errText); errMsg = ej.error?.message || ej.error?.code || errText; } catch {}
          console.error(`[chat/stream] Fallback failed: status=${fallbackResp.status}`);
...
app.put('/api/user-status', async (req, res) => {
  ...
  try {
    const cfg = await loadProjectConfig(projectId) || {};
    cfg._userStatus = (status || '').substring(0, 15);
    cfg._userStatusChanged = true;
    if (supabase) {
      await supabase.from('project_configs').upsert({
        project_id: projectId, config: cfg, updated_at: new Date().toISOString()
      }, { onConflict: 'project_id' });
      // Update cache
      if (_configsCache) { _configsCache[projectId] = cfg; _configsCacheTime = Date.now(); }
    }
    console.log('[api] User status updated:', projectId, '→', cfg._userStatus);
    res.json({ ok: true, userStatus: cfg._userStatus });
  } catch (e) {
    console.error('[api] PUT /api/user-status error:', e.message);
    res.status(500).json({ error: e.message });
  }
});
app.get('/api/user-status', async (req, res) => {
  ...
  try {
    const cfg = await loadProjectConfig(projectId) || {};
    res.json({ userStatus: cfg._userStatus || '', aiStatus: cfg._aiStatus || '' });
  } catch (e) {
    console.error('[api] GET /api/user-status error:', e.message);
    res.status(500).json({ error: e.message });
  }
});
...
像刚好想起这件事，自然地行动。不要用"通知""触发"等词。
  ...
  try {
    const llmResp = await internalApiFetch('/api/chat', {
      method: 'POST',
...
重要：标记用[[...]]包裹，放在末尾。一条回复 = 可选消息 + 最多一个标记。[[POKE]]无需内容。
...
async function dispatchNtfyForEvent(event) {
  ...
  const clickUrl = NTFY_CLICK_BASE_URL + '/?project=' + encodeURIComponent(event.projectId || '') +
    '&chat=' + encodeURIComponent(event.chatId || '');
  const result = await ntfyDispatcher({
    eventId: event.id,
  ...
  if (result.sent) {
    console.log('[ntfy] Notification accepted:', event.type, event.id);
  } else if (!result.skipped) {
    console.error('[ntfy] Notification failed:', event.type, event.id, result.error && result.error.message);
  }
  return result;
}
...
async function checkProjectDesires(pid, cfg) {
  ...
  try {
    ...
    if (usageEvent || data.usage) {
      ...
      internalApiFetch('/api/log-token-call', {
      ...
      try {
        ...
        if (actionType === 'status' && actions.status && typeof actions.status === 'string') {
          ...
          if (newStatus) {
            ...
            if (supabase) {
              await supabase.from('project_configs').upsert({
                project_id: pid, config: cfg, updated_at: nowISO
              }, { onConflict: 'project_id' });
              if (_configsCache) { _configsCache[pid] = cfg; _configsCacheTime = Date.now(); }
            }
            console.log('[cron] AI status updated:', pid, '→', newStatus);
          }
        }

        // A6: Poke ([[POKE]] content ignored; displays user status)
        if (actionType === 'poke') {
          const userStatus = cfg._userStatus || '';
          ...
          const { error: pokeErr } = await supabase.from('poke_events').insert({
            ...
            content: userStatus, source: 'ai', created_at: nowISO
          });
          if (pokeErr) {
            console.error('[cron] poke_events insert error:', pokeErr.message);
          } else {
            console.log('[cron] Proactive poke saved:', pokeId, '— userStatus:', userStatus);
          }
        }

        // A7: Email — handled by existing logic below
      } catch (businessErr) {
    ...
    if (emailSent) {
      await dispatchNtfyForEvent({
        id: savedEvent && savedEvent.id,
        projectId: pid,
        chatId,
        aiName: cfg.aiName || '暖伴',
        type: actionType,
        driveKey,
        content: (actionType === 'message' || actionType === 'todo')
          ? (cleanMessage || eventContent).slice(0, 200)
          : eventContent
      });
    } else {
      console.warn('[ntfy] Skipping email notification because email was not sent');
    }
    ...
    internalApiFetch('/api/projects/sync-configs', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
...
app.get('/api/toolkit/definitions', async (req, res) => {
  try {
    if (supabase) {
      ...
      return res.json({ definitions: sanitizeToolDefinitionsForClient(data.value || []) });
    }
...
// Render no longer serves the browser application. Vercel is the only public UI entry.
if (IS_RENDER) {
  app.use((req, res, next) => {
    if (req.path === '/healthz' || req.path === '/api' || req.path.startsWith('/api/')) return next();
    const target = new URL(APP_PUBLIC_ORIGIN);
    for (const key of ['project', 'chat']) {
